...
function setup() {
    ...
    var noteFrequencies = createNoteTable();
    volumeControl.addEventListener("change", changeVolume, false);
    noteFrequencies.forEach(function (notesInAnOctave, octave) {
        var notesAndFrequencies = Object.entries(notesInAnOctave);
        var octaveEl = document.createElement("div");
        octaveEl.className = "octave";
        notesAndFrequencies.forEach(function (_a) {
            var noteName = _a[0], noteFreq = _a[1];
 ...
 */
function createKey(note, freq, octave) {
    var keyElement = document.createElement("div");
    var labelElement = document.createElement("div");
    if (note.length === 1)
        keyElement.className = "naturalNote";
    if (note.length === 2)
        keyElement.className = "sharpOrFlatNote " + note;
...
function playTone(freq) {
    ...
    var waveType = wavePicker.options[wavePicker.selectedIndex].value;
    if (waveType === "custom")
...
function notePressed(event) {
    if (event.buttons & 1 || event instanceof KeyboardEvent) {
        var eventTarget = getHTMLElementForKeyboardEventCode(event.code) ||
            event.target;
        var className = eventTarget.className, dataset = eventTarget.dataset;
        // If the note is already playing, or eventTarget is not a note, return.
        if (className.includes("active") || !className.includes("Note"))
            return;
        eventTarget.className += " active"; // Style the key so that it looks pressed
        var octave = +dataset["octave"]; // The note wasn't already playing
...
function noteReleased(event) {
    var eventTarget = getHTMLElementForKeyboardEventCode(event.code) || event.target;
    ...
    if (dataset && eventTarget.className.includes("active")) {
        ...
        var octave = +dataset["octave"];
        var keysOscillator = oscillatorList[octave][dataset["note"]];
        keysOscillator.stop();
 ...
 */
function getHTMLElementForKeyboardEventCode(code) {
    switch (code) {
        case "KeyQ":
